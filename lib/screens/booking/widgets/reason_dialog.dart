...
class _ReasonDialogState extends State<ReasonDialog> {
  ...
  void saveHoldClick() async {
    ...
    await updateBooking(request).then((res) async {
      ...
      Map<String, dynamic> liveStreamRequest = {
        "inSeconds": "${(widget.status.booking_detail!.durationDiff.toInt() + request[BookingUpdateKeys.durationDiff].toString().toInt())}".toInt(),
  ...
  void _handleClick() {
    if (formKey.currentState!.validate()) {
      ...
      if (widget.status.booking_detail!.status == BookingStatusKeys.pending ||
          widget.status.booking_detail!.status == BookingStatusKeys.hold ||
          widget.status.booking_detail!.status == BookingStatusKeys.accept) {
        saveCancelClick();
