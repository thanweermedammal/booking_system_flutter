...
Future<void> saveUserData(UserData data) async {
  if (data.api_token.validate().isNotEmpty)
    await appStore.setToken(data.api_token.validate());
  ...
  if (data.loginType != "google")
    await appStore.setUserProfile(data.profile_image.validate());
  ...
  await appStore
      .setAddress(data.address != null ? data.address.validate() : '');
...
Future<DashboardResponse> userDashboard(
  ...
  if (isCurrentLocation) {
    _dashboardResponse = DashboardResponse.fromJson(
      await handleResponse(
        await buildHttpResponse(
            'dashboard-detail?latitude=$lat&longitude=$long&${appStore.isLoggedIn ? '?customer_id=${appStore.userId.validate()}' : ''}',
    ...
    _dashboardResponse = DashboardResponse.fromJson(await handleResponse(
        await buildHttpResponse(
            'dashboard-detail${appStore.isLoggedIn ? '?customer_id=${appStore.userId.validate()}' : ''}',
            method: HttpMethod.GET)));
  }
  ...
  if (_dashboardResponse.language_option != null) {
    setValue(SERVER_LANGUAGES,
        jsonEncode(_dashboardResponse.language_option!.toList()));
  }
  ...
  if (isCategoryWise) {
    if (isCurrentLocation) {
      return ServiceResponse.fromJson(await handleResponse(await buildHttpResponse(
          'service-list?per_page=$perPageItem&category_id=$categoryId&page=$page&customer_id=$customerId&latitude=$lat&longitude=$long',
          method: HttpMethod.GET)));
    } else {
      return ServiceResponse.fromJson(await handleResponse(await buildHttpResponse(
          'service-list?per_page=$perPageItem&category_id=$categoryId&page=$page&customer_id=$customerId',
          method: HttpMethod.GET)));
    }
  } else if (isSearch) {
    if (isCurrentLocation)
      return ServiceResponse.fromJson(await handleResponse(await buildHttpResponse(
          'service-list?per_page=$perPageItem&page=$page&customer_id=$customerId&search=$searchTxt&latitude=$lat&longitude=$long',
          method: HttpMethod.GET)));
    else
      return ServiceResponse.fromJson(await handleResponse(await buildHttpResponse(
          'service-list?per_page=$perPageItem&page=$page&customer_id=$customerId&search=$searchTxt',
          method: HttpMethod.GET)));
  } else {
    if (isCurrentLocation)
      return ServiceResponse.fromJson(await handleResponse(await buildHttpResponse(
          'service-list?per_page=$perPageItem&page=$page&customer_id=$customerId&latitude=$lat&longitude=$long',
          method: HttpMethod.GET)));
    else
      return ServiceResponse.fromJson(await handleResponse(await buildHttpResponse(
          'service-list?per_page=$perPageItem&page=$page&customer_id=$customerId',
          method: HttpMethod.GET)));
  }
}
//endregion
...
Future<CategoryResponse> getCategoryList(String page) async {
  return CategoryResponse.fromJson(await handleResponse(await buildHttpResponse(
      'category-list?page=$page&per_page=$perPageItem',
      method: HttpMethod.GET)));
}
//endregion
...
Future<WishListResponse> getWishlist() async {
  return WishListResponse.fromJson(await (handleResponse(
      await buildHttpResponse('user-favourite-service',
          method: HttpMethod.GET))));
}
